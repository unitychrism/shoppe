  ...
  class ProductsController < Shoppe::ApplicationController
    ...
    def index
      @products_paged = Shoppe::Product.root
                                       .includes(:translations, :locations, :stock_level_adjustments, :product_categories, :variants)
                                       .order(:name)
    ...
    def safe_params
      ...
      params[:product].permit(:name, :sku, :permalink, :description, :short_description, :weight, :price, :cost_price, :tax_rate_id, :stock_control, :active, :featured, :in_the_box, :vendor_id, :manufacturer_id, attachments: [default_image: file_params, data_sheet: file_params, extra: file_params], product_attributes_array: [:key, :value, :searchable, :public], product_category_ids: [])
    end
  end
end
